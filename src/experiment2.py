        ...
        for snap in snapshots:
            ...
            for AE in AEs:
                ...
                if turn_id == 0:
                    ...
                    state_utils.save_state(current_states,    f"{output_dir}/state_original_{AE.hidden_dim}.pt")
                    state_utils.save_state(compressed_states, f"{output_dir}/state_compressed_{AE.hidden_dim}.pt")
                    continue

                prev_original    = state_utils.load_state(f"{output_dir}/state_original_{AE.hidden_dim}.pt")
                prev_compressed  = state_utils.load_state(f"{output_dir}/state_compressed_{AE.hidden_dim}.pt")
                ...
                output, state_latency, state_ppl = evaluate_module.evaluate(
                    model, tokenizer, snap["new_text"],
                    copy.deepcopy(prev_original), device=device
                )
                compressed_output, compressed_latency, compressed_ppl = evaluate_module.evaluate(
                    model, tokenizer, snap["new_text"],
                    copy.deepcopy(reconstructed_states), device=device
                )

                if role == "assistant":
                    compressed_latency += decode_time

                    original_size_kb   = utils.get_memory_size_kb(f"{output_dir}/state_original_{AE.hidden_dim}.pt")
                    compressed_size_kb = utils.get_memory_size_kb(f"{output_dir}/state_compressed_{AE.hidden_dim}.pt")
